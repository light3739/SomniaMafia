 ...
 *
 * Shows session key status. Collapsible banner displayed in the bottom-left
          ...
          {hasSession ? (
            isLowBalance ? (
              ...
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${theme.divider} ${theme.footerBg}`}>
                <Sparkles className={`w-3.5 h-3.5 shrink-0 ${theme.accent}`} />
                <p className={`text-[10px] font-['Montserrat'] leading-snug ${theme.label}`}>
                  Auto-sign active. Every game action signs silently — no popup until the session expires.
                </p>
              </div>
            )
          ) : (
