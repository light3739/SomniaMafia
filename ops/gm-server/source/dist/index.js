...
import { getRoom, getPlayers, hasCommittedRole, getSessionKey, resolveNight, assertChainConfigOrThrow, getChainConfig, GM_ADDRESS, GamePhase, FLAGS, } from './chain.js';
...
async function verifyAuthorizedSignature(params) {
    const { roomId, signature, playerAddress, signerAddress, buildLegacyMessage, buildModernMessage, nonce, timestamp, chainId, } = params;
    ...
    if (normalizedSigner !== normalizedPlayer) {
        try {
            const session = await getSessionKey(normalizedPlayer, chainId);
...
app.post('/investigation-proof', async (req, res) => {
    try {
        ...
        if (signer !== detective) {
            const session = await getSessionKey(detective, chainId);
            const sessionAddress = String(session.sessionAddress || '').toLowerCase();
            const expiresAt = Number(session.expiresAt || 0);
            const sessionRoomId = Number(session.roomId || 0);
            const isActive = Boolean(session.isActive);
...
app.post('/night-action', async (req, res) => {
    try {
        ...
        const committed = await hasCommittedRole(rid, playerAddress, chainId);
        if (!committed) {
