const { createPublicClient, createWalletClient, http, parseEther, keccak256, encodeAbiParameters, parseAbiParameters, getAddress } = require('viem');
...
async function run() {
    const hostPk = "0xREDACTED_LEAKED_KEY";
    ...
    const waitForPhase = async (roomId, expectedPhase) => {
        console.log(`Waiting for phase ${expectedPhase}...`);
        while (true) {
            const roomInfo = await client.readContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'rooms', args: [roomId] });
            console.log(`Current Phase: ${roomInfo[3]}`);
            if (roomInfo[3] === expectedPhase) break;
            await wait(5000);
        }
        await wait(10000); // Extra buffer for all nodes to sync
    };
    ...
    for (let i = 0; i < 4; i++) {
        ...
        await wal.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'joinRoom', args: [roomId, `Bot_${i}`, "0x00", "0x0000000000000000000000000000000000000000"] });
        console.log(`Bot ${i} joined.`);
    }
    const allPlayers = [{ acc: hostAccount, wal: hostWallet }, ...bots];

    // Phases
    await hostWallet.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'startGame', args: [roomId] });
    console.log("Game started.");

    await waitForPhase(roomId, 1); // SHUFFLING
    ...
    for (let i = 0; i < 5; i++) {
        await allPlayers[i].wal.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'commitDeck', args: [roomId, deckHash] });
        await allPlayers[i].wal.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'revealDeck', args: [roomId, dummyDeck, dummySalt] });
        console.log(`Player ${i} shuffled.`);
    }

    await waitForPhase(roomId, 2); // REVEAL

    // Reveal/Keys
    const addresses = allPlayers.map(p => p.acc.address);
    for (let i = 0; i < 5; i++) {
        const recipients = addresses.filter(a => a !== allPlayers[i].acc.address);
        await allPlayers[i].wal.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'shareKeysToAll', args: [roomId, recipients, recipients.map(() => "0x00")] });
        await allPlayers[i].wal.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'commitAndConfirmRole', args: [roomId, keccak256("0x00")] });
        console.log(`Player ${i} confirmed role.`);
    }

    await waitForPhase(roomId, 3); // DAY
    await hostWallet.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'startVoting', args: [roomId] });
    await waitForPhase(roomId, 4); // VOTING
    ...
    await waitForPhase(roomId, 5); // NIGHT
    ...
    await hostWallet.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'commitNightAction', args: [roomId, actionHash] });
    console.log("Action committed.");

    const revealHash = await hostWallet.writeContract({ address: MAFIA_CONTRACT_ADDRESS, abi: MAFIA_ABI, functionName: 'revealNightAction', args: [roomId, ACTION_CHECK, target, nightSalt] });
    await client.waitForTransactionReceipt({ hash: revealHash });
    console.log("Action revealed!");
    ...
    console.log("API Result Status:", apiRes.status);
    console.log("API Result Body:", JSON.stringify(result));
}
run();
